...
The project was built for the practical abuse patterns VPS providers often see from upstreams such as Hetzner: port scanning, CDN clean-IP scanning, infected VPN-client traffic, outbound spam attempts, brute-force fanout, exposed Telnet, BitTorrent/P2P abuse, and web exploit probing. Its goal is to block abusive traffic leaving the VM before it reaches the Internet.
...
- Users can run Cloudflare/Fastly/CloudFront clean-IP scanners that create upstream abuse reports.
...
- CDN IP range refresh for Cloudflare, Fastly, and CloudFront.
...
- Generic outbound port scans and net scans.
- Cloudflare/Fastly/CloudFront clean-IP scanning.
- Repeated TLS/SNI matrix probing against CDN edge ranges.
- Cloudflare alternate-port scanning, including common HTTP/HTTPS edge ports.
- Cloudflare WARP-style UDP probing on `2408`.
...
- Many unique CDN destination IPs on `443` or alternate CDN ports: quarantine for CDN clean-IP scanning.
...
After publishing the repository, VM templates can install it with a single shell command by cloning the public repo and running the installer:

```bash
sudo git clone https://github.com/YOUR_ORG/inside-vm-egress-guard.git /opt/inside-vm-egress-guard && cd /opt/inside-vm-egress-guard && sudo ./install.sh
```

Replace `YOUR_ORG` with your GitHub organization or account name.
...
```bash
systemctl status nftables suricata egress-guardd anti-abuse-restore.timer cloudflare-ip-refresh.timer
```
...
```bash
sudo systemctl disable --now anti-abuse-restore.timer cloudflare-ip-refresh.timer egress-guardd
